...
const SendParcel = () => {
  ...
  const { user } = useAuth();
  const { updateTrackingStatus, isLoading: trackingLoading } = useParcelTracking();
  ...
  const calculateCost = (data) => {
    ...
    let baseCost = 0, extraCost = 0, interDistrictSurcharge = 0;
    ...
    return { baseCost, extraCost, interDistrictSurcharge, totalCost: baseCost + extraCost + interDistrictSurcharge };
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.data?.insertedId) {
        // Automatically create tracking for "submitted"
        updateTrackingStatus({
          parcelId: res.data.insertedId, status: "submitted", updatedBy: user.email, details: `Parcel submitted by ${user.displayName}` });
  ...
  const handleSenderRegionChange = (region) => {
    const options = regionsData.filter((r) => r.region === region).map((r) => r.district);
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {type === "non-document" && (
              <div>
                ...
                <input type="number" step="0.1" {...register("weight", { required: "Weight is required for non-documents", min: 0 })} className="input input-bordered w-full" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="border border-white rounded-xl p-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <select {...register("sender_region", { required: true })} onChange={(e) => handleSenderRegionChange(e.target.value)} className="select select-bordered w-full">
                <option value="">Select Region</option>
                {[...new Set(regionsData.map((r) => r.region))].map((region) => <option key={region} value={region}>{region}</option>)}
              </select>
              <select {...register("sender_service_center", { required: true })} className="select select-bordered w-full">
                <option value="">Select Service Center</option>
                {senderServiceCenters.map((center, i) => <option key={i} value={center}>{center}</option>)}
              </select>
              <input {...register("sender_address", { required: true })} placeholder="Address" className="input input-bordered w-full" />
              <textarea {...register("pickup_instruction")} placeholder="Pickup Instructions" className="textarea textarea-bordered w-full" />
            </div>
          </div>
          ...
          <div className="border border-white rounded-xl p-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <select {...register("receiver_region", { required: true })} onChange={(e) => handleReceiverRegionChange(e.target.value)} className="select select-bordered w-full">
                <option value="">Select Region</option>
                {[...new Set(regionsData.map((r) => r.region))].map((region) => <option key={region} value={region}>{region}</option>)}
              </select>
              <select {...register("receiver_service_center", { required: true })} className="select select-bordered w-full">
                <option value="">Select Service Center</option>
                {receiverServiceCenters.map((center, i) => <option key={i} value={center}>{center}</option>)}
              </select>
              <input {...register("receiver_address", { required: true })} placeholder="Address" className="input input-bordered w-full" />
              <textarea {...register("delivery_instruction")} placeholder="Delivery Instructions" className="textarea textarea-bordered w-full" />
            </div>
          </div>
        </div>

        <div className="text-center">
          <button type="submit" className="btn btn-primary text-black" disabled={trackingLoading}>
            {trackingLoading ? "Submitting..." : "Submit"}
          </button>
        </div>
      </form>
    </div>
