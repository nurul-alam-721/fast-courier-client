...
const AssignRider = () => {
  ...
  const { data: parcels = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        console.log("Fetching parcels from /parcels/paid-pending");
        const res = await axiosSecure.get("/parcels/paid-pending");
        console.log("Parcels response:", res.data);
        return res.data;
      } catch (err) {
        console.error("Error fetching parcels:", err.response?.data || err.message);
        throw err;
      }
    },
    retry: 0,
  ...
  const { data: riders = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.error("Error fetching riders:", err.response?.data || err.message);
        throw err;
      }
    },
    retry: 0,
  ...
  const filteredRiders = selectedParcel 
    ? riders.filter(rider => rider.district === selectedParcel.sender_service_center)
    : [];

  // Mutation for assigning rider
  const assignMutation = useMutation({
    mutationFn: async ({ parcelId, rider }) => {
      console.log("Assigning rider to parcel ID:", parcelId, "Rider:", rider);
    ...
    onError: (err) => {
      ...
      Swal.fire("Error!", `Failed to assign rider: ${err.response?.data?.message || err.message}`, "error");
    },
  });
  ...
  if (error) {
    ...
    return <p>Error loading parcels: {error.response?.data?.message || error.message}</p>;
  }
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <div className="mb-6 bg-gray-50 p-4 rounded-lg">
              <p className="font-semibold text-gray-900 mb-1">
                Parcel ID: <span className="text-blue-600">{selectedParcel.tracking_id || "N/A"}</span>
              </p>
              <p className="text-gray-800">
                Service Center: <span className="font-medium">{selectedParcel.sender_service_center || "N/A"}</span>
              </p>
            </div>
            
            <div className="mb-6">
              <h4 className="font-semibold text-gray-900 mb-3 text-lg">Available Riders</h4>
              
              {filteredRiders.length === 0 ? (
                <div className="text-center py-6 bg-gray-100 rounded-lg border border-gray-300">
                  <p className="text-gray-800 font-medium">No riders found for this service center</p>
                  <p className="text-gray-600 text-sm mt-1">Please check back later or contact support</p>
                </div>
              ) : (
                <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-lg">
                  {filteredRiders.map((rider) => (
                    ...
                    >
                      <div>
                        <p className="font-bold text-gray-900">{rider.name || "N/A"}</p>
                        <p className="text-gray-800">{rider.email || "N/A"}</p>
                        <p className="text-gray-700 text-sm">{rider.district || "N/A"}</p>
                      </div>
                      <button
                        className="btn btn-sm bg-green-600 hover:bg-green-700 text-white font-medium"
                        onClick={() => {
                          if (!selectedParcel?._id) {
                            return Swal.fire("Error", "Parcel ID is missing", "error");
                          }
                          if (!isValidObjectId(selectedParcel._id)) {
                            return Swal.fire("Error", "Invalid Parcel ID format", "error");
                          }
                          
                          Swal.fire({
                            title: "Confirm Assignment",
                            text: `Assign ${rider.name || "this rider"} to this parcel?`,
                            icon: "question",
